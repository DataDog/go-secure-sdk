...
func TestOSFS(t *testing.T) {
	...
	t.Run("file operations", func(t *testing.T) {
		...
		if runtime.GOOS == "windows" {
			require.Equal(t, fi.Mode().String(), "-rw-rw-rw-")
		} else {
		...
		if runtime.GOOS == "windows" {
			require.Equal(t, fi2.Mode().String(), "-rw-rw-rw-")
		} else {
		...
		if runtime.GOOS == "windows" {
			require.Equal(t, fi3.Mode().String(), "-rw-rw-rw-")
		} else {
		...
		if runtime.GOOS == "windows" {
			require.Equal(t, fi5.Mode().String(), "-rw-rw-rw-")
		} else {
		...
		if runtime.GOOS == "windows" {
			require.Equal(t, fio.Mode().String(), "-rw-rw-rw-")
		} else {
	...
	t.Run("directory operations", func(t *testing.T) {
		...
		require.NoError(t, sysFs.WalkDir(tmpDir, func(path string, d fs.DirEntry, err error) error {
			...
			if d.IsDir() {
				names = append(names, fmt.Sprintf("d %s", filepath.ToSlash(path)))
			} else {
				names = append(names, fmt.Sprintf("f %s", filepath.ToSlash(path)))
			}
