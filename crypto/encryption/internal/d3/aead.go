...
// D3 provides file content encryption/decryption algorithm based on
// HKDF_SHA256 for Key wrapping and AES-GCM to accomplish encryption for
...
func EncryptedLength(plaintextLength int) int {
	// ContentLength (4B) || Content (*B) || Tag (16B)
	dataChunkSize := chunkSize - gcmTagSize - encodedContentLength
	dataChunkCount := int(float64(plaintextLength) / float64(dataChunkSize))
	dataTrailer := plaintextLength % dataChunkSize

	switch {
	case dataTrailer > 0:
		return headerSize + dataChunkCount*chunkSize + encodedContentLength + dataTrailer + gcmTagSize
	default:
...
func encrypt(rand io.Reader, key []byte, plaintext io.Reader, aad []byte, out io.Writer) error {
	...
	if len(key) < minKeyLen {
		return fmt.Errorf("key must be %d bytes long at least", minKeyLen)
	}
	if plaintext == nil {
	...
	// Derive all key materials from the nonce (maybe using a DRBG could be faster?)
	var eK [encryptionKeyLen + encryptionNonceLen]byte
	h := hkdf.New(sha256.New, key, nonce[:], []byte("datadog-chunk-encryption-keys-v1"))
	...
	if err != nil {
		return fmt.Errorf("unable to initialize block cipher: %w", err)
	}

	// Use AES-GCM block mode
	aead, err := cipher.NewGCMWithNonceSize(b, encryptionNonceLen)
	if err != nil {
		return fmt.Errorf("unable to initial AEAD: %w", err)
	}
	...
	for {
		...
		nhm.Reset()
		nhm.Write([]byte("datadog-chunked-encryption-nonce-v1")) // Purpose
		...
		chunkAad, err := canonicalization.PreAuthenticationEncoding(
			[]byte("datadog-chunked-encryption-aad-v1"),
...
func decrypt(key []byte, ciphertext io.Reader, aad []byte, out io.Writer) error {
	...
	for {
		...
		// Extract plaintext
		contentLength := binary.BigEndian.Uint32(chunkRaw[:encodedContentLength])
