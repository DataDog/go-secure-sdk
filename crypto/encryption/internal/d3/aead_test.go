...
func Test_encrypt(t *testing.T) {
	...
	tests := []struct {
		...
		// ---------------------------------------------------------------------
		{
			name: "valid",
			args: args{
				r:         strings.NewReader("deterministic-nonce-entropy-source"), // Deterministic entropy source
				key:       []byte("sQU8SWrSiaz0eewSS9INE1gDGv1nULsB"),
				plaintext: strings.NewReader(`{"state":"gInhnZvpFdKJ2pg7gwPeVuKFZJMNUNNo"}`),
			},
			wantErr: false,
			want: []byte{
				0xd3, 0x64, 0x65, 0x74, 0x65, 0x72, 0x6d, 0x69, 0x6e, 0x69, 0x73, 0x74, 0x69, 0x63, 0x2d, 0x6e, //  |.deterministic-n|
				0x6f, 0x6e, 0x63, 0x65, 0x2d, 0x65, 0x6e, 0x74, 0x72, 0x6f, 0x70, 0x79, 0x2d, 0x73, 0x6f, 0x75, //  |once-entropy-sou|
				0x72, 0x01, 0x23, 0x53, 0xc6, 0x21, 0x81, 0x9d, 0x11, 0x2e, 0xa8, 0x71, 0xdf, 0xd0, 0x51, 0xd8, //  |r.#S.!.....q..Q.|
				0x5b, 0xbe, 0x38, 0x63, 0x35, 0xa3, 0xb8, 0x14, 0xe4, 0xae, 0x17, 0x23, 0x2a, 0x32, 0x54, 0x53, //  |[.8c5......#*2TS|
				0x80, 0x92, 0x02, 0x67, 0x3e, 0x93, 0x0c, 0x1e, 0x83, 0x56, 0x5f, 0x57, 0xc0, 0x38, 0x65, 0xe2, //  |...g>....V_W.8e.|
				0x51, 0xb7, 0xe6, 0x9d, 0x15, 0x39, 0x51, 0x45, 0x78, 0x94, 0x1a, 0xff, 0xd1, 0x05, 0xb8, 0xfa, //  |Q....9QEx.......|
				0x25, 0x93, //  |%.|
			},
		},
		{
			...
			want: []byte{
				...
				0x5b, 0xbe, 0x38, 0x63, 0x35, 0xa3, 0xb8, 0x14, 0xe4, 0xae, 0x17, 0x23, 0x2a, 0x32, 0x54, 0x53, //  |[.8c5......#*2TS|
				0x80, 0x92, 0x02, 0x67, 0x3e, 0x93, 0x0c, 0x1e, 0x83, 0x56, 0x5f, 0x57, 0xc0, 0x38, 0x65, 0xe2, //  |...g>....V_W.8e.|
				0x51, 0xb7, 0x8a, 0xb8, 0x37, 0x01, 0xa9, 0x33, 0x15, 0xfe, 0xf3, 0x72, 0xbf, 0x5a, 0x2a, 0xa3, //  |Q...7..3...r.Z*.|
				0xf1, 0xc6, //  |..|
			},
		},
	}
...
func TestEncryptDecrypt_256(t *testing.T) {
	...
	_, err := io.Copy(&plaintext, io.LimitReader(rand.Reader, 256))
	require.NoError(t, err)

	masterKey := []byte(`hbHt9eWxCN/p0Z#LJmXMb*qV2v;OwY_Db+3^uWS;vz\$7%BW.#LU8Q))n=#pOh+`)

	var encrypted bytes.Buffer
	require.NoError(t, encrypt(rand.Reader, masterKey, bytes.NewReader(plaintext.Bytes()), nil, &encrypted))
...
func TestEncryptDecrypt_1MB(t *testing.T) {
	...
	// Generate a 1MB file content
	var plaintext bytes.Buffer
	_, err := io.Copy(&plaintext, io.LimitReader(rand.Reader, 1<<20))
	require.NoError(t, err)
