...
func VerifiableUUIDGenerator(source UUIDGeneratorFunc, secretKey []byte) Generator {
	return &verifiableUUIDGenerator{
...
type verifiableUUIDGenerator struct {
	randReader io.Reader
	secretKey  []byte
	source     UUIDGeneratorFunc
}

func (vu *verifiableUUIDGenerator) Generate(opts ...GenerateOption) (string, error) {
	...
	var authKey [32]byte
	h := hkdf.New(sha256.New, vu.secretKey, nonce[:], []byte("datadog-uuid-wrapper-mac-v1"))
	if _, err := io.ReadFull(h, authKey[:]); err != nil {
		return "", fmt.Errorf("unable to derive authentication key: %w", err)
	}

	// Prepare protected
	protected := []byte("datadog-uuid-protected-token-v1")
...
func (vu *verifiableUUIDGenerator) Verify(in string) error {
	_, err := vu.Extract(in)
	return err
}

func (vu *verifiableUUIDGenerator) Extract(in string) ([]byte, error) {
	// Detect prefix usage
	var prefix string
	...
	// threaten all generated tokens in the potential case of a secret leak.
	var authKey [32]byte
	h := hkdf.New(sha256.New, vu.secretKey, sig[:12], []byte("datadog-uuid-wrapper-mac-v1"))
	if _, err := io.ReadFull(h, authKey[:]); err != nil {
